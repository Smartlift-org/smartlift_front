...
const MemberSelectionScreen: React.FC<MemberSelectionScreenProps> = ({ navigation }) => {
  ...
  const { routineId, customName } = route.params as { routineId: string, customName: string };
  ...
  const loadMembers = async (id: string, page: number = 1, search: string = searchQuery) => {
    if (!id) return;
    
    try {
      setLoading(true);
      const response = await trainerService.getMembers(id, page, 10, { search });
  ...
  const handleAssignRoutine = async (memberId: string) => {
    try {
      ...
      await trainerService.assignRoutine(trainerId, memberId, routineId, customName);
      ...
      // Navegar al perfil del miembro con un parámetro de refresco
      navigation.navigate("MemberProfile", { memberId, refresh: true });
      
      // También actualizar el dashboard del entrenador para reflejar los cambios
      ...
      <View className="flex-1 px-4 pb-4">
        ...
        {loading && !refreshing ? (
          <View className="flex-1 justify-center items-center">
            ...
            <Text className="mt-2.5 text-base text-gray-700">Cargando miembros...</Text>
          </View>
        ) : (
          <>
            {members.length === 0 ? (
              <View className="flex-1 justify-center items-center pb-24">
                ...
                <Text className="text-xl text-gray-600 mt-4 font-semibold">No tienes miembros</Text>
                ...
                >
                  <Text className="text-white font-semibold text-base">Gestionar Miembros</Text>
                </TouchableOpacity>
              </View>
            ) : (
              <FlatList
