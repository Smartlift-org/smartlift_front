...
const PrivacySettingsScreen: React.FC = () => {
  ...
  const loadPrivacySettings = async () => {
    try {
      ...
      AppAlert.error(error instanceof Error ? error.message : 'Error al cargar configuración');
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateSetting = (key: keyof PrivacySettings, value: boolean) => {
    ...
    // If making profile private, turn off all visibility settings
    if (key === 'is_profile_public' && !value) {
      setSettings(prev => ({
        ...prev,
        is_profile_public: false,
        show_name: true, // Keep some defaults
        show_profile_picture: true,
        show_workout_count: true,
        show_join_date: false,
        show_personal_records: false,
  ...
  const saveSettings = async () => {
    try {
      setSaving(true);
      const response = await publicProfileService.updatePrivacySettings(settings);
      
      if (response.success) {
        setHasChanges(false);
        AppAlert.success('Configuración actualizada exitosamente');
      }
    } catch (error) {
      AppAlert.error(error instanceof Error ? error.message : 'Error al guardar configuración');
    } finally {
      setSaving(false);
    }
  };
  ...
  const renderSettingRow = (
    ...
    <View className={`bg-white px-4 py-4 border-b border-gray-100 ${disabled ? 'opacity-50' : ''}`}>
  ...
  if (loading) {
    return (
      <View className="flex-1 bg-gray-50">
        <ScreenHeader title="Configuración de Privacidad" onBackPress={handleBackPress} />
      ...
      <ScrollView className="flex-1">
        ...
        <View className="mt-4">
          <View className="bg-white px-4 py-3 border-b border-gray-200">
            <Text className="text-lg font-semibold text-gray-800">Visibilidad del Perfil</Text>
        ...
        <View className="mt-6">
          <View className="bg-white px-4 py-3 border-b border-gray-200">
            <Text className="text-lg font-semibold text-gray-800">Información del Perfil</Text>
            <Text className="text-sm text-gray-600 mt-1">
        ...
        <View className="mt-6">
          <View className="bg-white px-4 py-3 border-b border-gray-200">
            <Text className="text-lg font-semibold text-gray-800">Información de Actividad</Text>
            <Text className="text-sm text-gray-600 mt-1">
          ...
          <View className="flex-row">
            ...
            <View className="flex-1 ml-3">
              ...
              <Text className="text-sm text-blue-700 mt-1">
                Solo los datos que elijas mostrar serán visibles para otros usuarios. 
        ...
        {hasChanges && (
          <View className="mx-4 mt-6 mb-8">
            <TouchableOpacity
              className={`bg-blue-600 rounded-lg py-4 items-center ${saving ? 'opacity-50' : ''}`}
              onPress={saveSettings}
              disabled={saving}
            >
              {saving ? (
