...
const TrainerRoutinesScreen: React.FC<TrainerRoutinesScreenProps> = ({ navigation }) => {
  ...
  const [selectedRoutine, setSelectedRoutine] = useState<TrainerRoutine | null>(null);
  ...
  const loadRoutines = async (id: string, page: number = 1, difficulty: string = difficultyFilter) => {
    if (!id) return;
    
    try {
      setLoading(true);
      const response = await trainerService.getRoutines(id, page, 10, difficulty);
    ...
    <View className="bg-white rounded-xl mb-4 shadow flex-row">
      ...
      <View className="p-2 justify-center items-center border-l border-gray-100">
        ...
        <TouchableOpacity 
          ...
          onPress={() => navigation.navigate("RoutineEdit", { routineId: item.id, refresh: true })}
        >
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#f8f9fa]" edges={["top"]}>
      ...
      {/* Modal para personalizar nombre de rutina antes de asignar */}
      <Modal
        transparent={true}
