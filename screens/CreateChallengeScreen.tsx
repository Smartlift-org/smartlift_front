...
const CreateChallengeScreen: React.FC<CreateChallengeScreenProps> = ({ navigation, route }) => {
  ...
  const validateForm = (): string | null => {
    ...
    if (selectedExercises.length === 0) return "Debes agregar al menos un ejercicio";
    
    for (let i = 0; i < selectedExercises.length; i++) {
      const ex = selectedExercises[i];
      if (ex.sets < 1 || ex.sets > 20) return `Las series deben estar entre 1 y 20 (Ejercicio ${i + 1})`;
      if (ex.reps < 1 || ex.reps > 100) return `Las repeticiones deben estar entre 1 y 100 (Ejercicio ${i + 1})`;
      if (ex.rest_time_seconds < 0 || ex.rest_time_seconds > 600) return `El descanso debe estar entre 0 y 600 segundos (Ejercicio ${i + 1})`;
    }
  ...
  const handleCreateChallenge = async () => {
    ...
    try {
      ...
      navigation.goBack(); // Volver a la lista de desafíos 
    } catch (error: any) {
  ...
  const renderSelectedExercise = (exercise: SelectedExercise, index: number) => (
    <View key={index} className="bg-white rounded-lg p-4 mb-4 shadow-sm border border-gray-200">
      <View className="flex-row justify-between items-center mb-3">
        <Text className="font-bold text-gray-900 flex-1">{exercise.exercise.name}</Text>
        <TouchableOpacity
  ...
  const renderExerciseSelector = () => {
    ...
    return (
      <SafeAreaView className="absolute inset-0 bg-gray-50 z-10" edges={["top"]}>
        ...
        {loadingExercises ? (
          ...
          <FlatList
            data={availableExercises.filter(ex => !selectedExercises.some(sel => sel.exercise_id === ex.id))}
    ...
    <>
      ...
      <ScrollView className="flex-1">
        ...
        <View className="bg-white rounded-lg p-4 mb-4 shadow-sm">
          <Text className="text-lg font-bold text-gray-900 mb-4">Información del Desafío</Text>
          ...
          <View className="flex-row justify-between mb-4">
            <View className="flex-1 mr-2">
              <Text className="text-sm text-gray-600 mb-1">Duración estimada (min)</Text>
              <TextInput
                className="border border-gray-300 rounded-lg px-3 py-3"
                  ...
                  renderItem={({ item: [level, data] }) => (
                    <TouchableOpacity
                      className={`px-3 py-2 rounded mr-2 ${
                        difficultyLevel === parseInt(level) ? "bg-blue-500" : "bg-gray-200"
                      }`}
                      onPress={() => setDifficultyLevel(parseInt(level) as 1 | 2 | 3 | 4 | 5)}
                    >
                      <Text className={`text-xs ${
                        difficultyLevel === parseInt(level) ? "text-white" : "text-gray-700"
                      }`}>
                        {data.emoji} {level}
                      </Text>
                    </TouchableOpacity>
          ...
          <View className="bg-blue-50 p-3 rounded-lg">
            <Text className="text-blue-800 text-sm">
              ℹ️ El desafío será activo por 7 días desde hoy y estará disponible automáticamente para todos tus usuarios.
            </Text>
          </View>
        </View>
