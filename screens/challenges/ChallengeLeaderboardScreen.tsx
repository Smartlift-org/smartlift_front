...
const ChallengeLeaderboardScreen: React.FC<ChallengeLeaderboardScreenProps> = ({
  ...
  const [leaderboardData, setLeaderboardData] =
    useState<ChallengeLeaderboard | null>(null);
  ...
  const renderLeaderboardItem = ({ item }: { item: LeaderboardEntry }) => {
    ...
    const isCurrentUser =
      leaderboardData?.user_best_attempt?.user?.id === item.user.id;
          ...
          <View className="flex-row items-center flex-1">
            ...
            <View className="flex-1">
              ...
              <Text className="text-sm text-gray-600">
                Completado el{" "}
                {item.completed_at
                  ? formatChallengeDate(item.completed_at)
                  : "N/A"}
              </Text>
            </View>
          </View>
    ...
    <>
      ...
      <SafeAreaView className="flex-1 bg-gray-50" edges={["top"]}>
        <View className="flex-1">
          ...
          <FlatList
            data={
              leaderboardData.leaderboard?.filter((item) => item?.user?.id) ||
              []
            }
            renderItem={renderLeaderboardItem}
            keyExtractor={(item, index) =>
              `${item.user?.id || index}-${item.position || index}`
            }
                  ...
                  <View className="flex-row justify-around">
                    ...
                    <View className="items-center">
                      <Text className="text-2xl font-bold text-green-600">
                        {leaderboardData.total_completed ??
