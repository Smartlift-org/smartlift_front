...
type Props = {
  navigation: NativeStackNavigationProp<RootStackParamList, "ModifiedRoutineResult">;
  route: RouteProp<RootStackParamList, "ModifiedRoutineResult">;
};

const ModifiedRoutineResultScreen: React.FC<Props> = ({ navigation, route }) => {
  const { originalRoutine, modifiedRoutine, appliedModifications } = route.params;
  ...
  const renderRoutineCard = (routine: AIRoutine, title: string, isModified: boolean = false) => (
    <View className={`p-4 rounded-lg border-2 ${isModified ? 'border-green-200 bg-green-50' : 'border-gray-200 bg-white'} mb-4`}>
      ...
      <Text className="text-xl font-bold text-gray-900 mb-2">{routine.name}</Text>
      ...
      <View className="border-t border-gray-200 pt-3">
        ...
        {routine.routine_exercises.slice(0, 3).map((exercise: any, index: number) => (
          <View key={exercise.id} className="flex-row justify-between items-center py-1">
  ...
  const renderChangesApplied = () => (
    <View className="p-4">
      <Text className="text-lg font-semibold text-gray-800 mb-4">Cambios Aplicados</Text>
      ...
      {appliedModifications.difficulty && (
        <View className="mb-3 p-3 bg-blue-50 rounded-lg">
          ...
          <Text className="text-blue-700">{appliedModifications.difficulty}</Text>
        </View>
      )}
      ...
      {appliedModifications.duration && (
        <View className="mb-3 p-3 bg-purple-50 rounded-lg">
          ...
          <Text className="text-purple-700">{appliedModifications.duration}</Text>
        </View>
      )}

      {appliedModifications.exercisesAdded && appliedModifications.exercisesAdded.length > 0 && (
        <View className="mb-3 p-3 bg-green-50 rounded-lg">
          <View className="flex-row items-center mb-1">
            <FontAwesome5 name="plus" size={16} color="#10B981" />
            <Text className="ml-2 font-medium text-green-800">Ejercicios Agregados</Text>
          </View>
          {appliedModifications.exercisesAdded.map((exercise: string, index: number) => (
            <Text key={index} className="text-green-700">• {exercise}</Text>
          ))}
        </View>
      )}

      {appliedModifications.exercisesReplaced && appliedModifications.exercisesReplaced.length > 0 && (
        <View className="mb-3 p-3 bg-yellow-50 rounded-lg">
          <View className="flex-row items-center mb-1">
            <FontAwesome5 name="exchange-alt" size={16} color="#F59E0B" />
            <Text className="ml-2 font-medium text-yellow-800">Ejercicios Reemplazados</Text>
          </View>
          {appliedModifications.exercisesReplaced.map((replacement: string, index: number) => (
            <Text key={index} className="text-yellow-700">• {replacement}</Text>
          ))}
        </View>
      )}
      ...
      {appliedModifications.volumeChanges && (
        <View className="mb-3 p-3 bg-indigo-50 rounded-lg">
          ...
          <Text className="text-indigo-700">{appliedModifications.volumeChanges}</Text>
        </View>
      )}
      ...
      {appliedModifications.restTimeChanges && (
        <View className="mb-3 p-3 bg-orange-50 rounded-lg">
          ...
          <Text className="text-orange-700">{appliedModifications.restTimeChanges}</Text>
        </View>
      )}
    </View>
  );
  ...
  const renderDetailedComparison = () => (
    <View className="p-4">
      <Text className="text-lg font-semibold text-gray-800 mb-4">Comparación Detallada</Text>
      ...
      <View className="bg-white rounded-lg border border-gray-200">
        ...
        <View className="flex-row border-b border-gray-200">
          ...
          <View className="flex-1 p-3 border-r border-gray-200">
            <Text className="text-gray-600">{originalRoutine.duration} min</Text>
          </View>
          <View className="flex-1 p-3">
            <Text className="text-green-600 font-medium">{modifiedRoutine.duration} min</Text>
          </View>
        </View>
        ...
        <View className="flex-row border-b border-gray-200">
          ...
          <View className="flex-1 p-3">
            <Text className={`${getDifficultyColor(modifiedRoutine.difficulty)} font-medium`}>
        ...
        <View className="flex-row">
          ...
          <View className="flex-1 p-3 border-r border-gray-200">
            <Text className="text-gray-600">{originalRoutine.routine_exercises.length}</Text>
          </View>
          <View className="flex-1 p-3">
            <Text className="text-green-600 font-medium">{modifiedRoutine.routine_exercises.length}</Text>
          </View>
        </View>
      </View>
    </View>
    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      <ScrollView className="flex-1">
        ...
        <View className="bg-white">
          <View className="flex-row border-b border-gray-200">
            <TouchableOpacity
              ...
              className={`flex-1 py-4 items-center border-b-2 ${
                activeTab === 'comparison' ? 'border-blue-500' : 'border-transparent'
              }`}
            >
          ...
          {/* Contenido de tabs */}
          {activeTab === 'comparison' ? renderChangesApplied() : renderDetailedComparison()}
        </View>
      </ScrollView>
      ...
      <View className="p-4 bg-white border-t border-gray-200">
        <View className="flex-row space-x-3">
          ...
          >
            <View className="flex-row items-center">
              ...
              <Text className="text-white font-semibold ml-2">Aceptar Rutina</Text>
            </View>
          </TouchableOpacity>
        </View>
        ...
        >
          <Text className="text-blue-600 font-medium">Modificar Nuevamente</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
