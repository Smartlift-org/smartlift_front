...
const WorkoutInProgressScreen: React.FC<WorkoutInProgressScreenProps> = ({
  navigation,
  route,
}) => {
  ...
  const hasNextExercise =
    currentExerciseIndex < workout.routine.exercises.length - 1;
          ...
          <View className="flex-row justify-between mt-8">
            <TouchableOpacity
              className={`flex-row items-center py-2 px-4 rounded-lg ${
                hasPreviousExercise ? "bg-gray-200" : "bg-gray-100"
              }`}
              onPress={() => {
            ...
            >
              <Ionicons
                name="arrow-back"
                size={20}
                color={hasPreviousExercise ? "#4f46e5" : "#9ca3af"}
              />
              <Text
                className={`ml-2 ${
                  hasPreviousExercise ? "text-indigo-600" : "text-gray-400"
                }`}
              >
                Anterior
              </Text>
            ...
            <TouchableOpacity
              className={`flex-row items-center py-2 px-4 rounded-lg ${
                hasNextExercise ? "bg-gray-200" : "bg-gray-100"
              }`}
              onPress={() => {
                if (hasNextExercise) {
                  setCurrentExerciseIndex(currentExerciseIndex + 1);
            ...
            >
              <Text
                className={`mr-2 ${
                  hasNextExercise ? "text-indigo-600" : "text-gray-400"
                }`}
              >
                Siguiente
              </Text>
              <Ionicons
                name="arrow-forward"
                size={20}
                color={hasNextExercise ? "#4f46e5" : "#9ca3af"}
              />
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View className="bg-white rounded-lg shadow-sm p-4 mt-4">
          ...
          <View className="h-32 bg-gray-100 rounded-lg justify-center items-center">
            <Text className="text-gray-600">
              Aquí iría la interfaz de registro de series
            </Text>
          </View>
        </View>
      </ScrollView>
